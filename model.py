...
class ResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, resample=None, hw=DIM):
        ...
        self.relu1 = nn.ReLU()
        self.relu2 = nn.ReLU()
        # Residual blocks include two convolutional layers with batch normalization before each convolutional layer.
        if resample == 'down':
            # LayerNorm([C, H, W]) applies Layer Normalization over a mini-batch of inputs
...
# using a decoder network, reconstructs the image
class Decoder(nn.Module):
    def __init__(self, dim=DIM, output_dim=OUTPUT_DIM):
        ...
        self.ln1 = nn.Linear(LATENT_SIZE, 4*4*8*self.dim)
        self.rb1 = ResidualBlock(8*self.dim, 8*self.dim, 3, resample='up')
        self.rb2 = ResidualBlock(8*self.dim, 4*self.dim, 3, resample='up')
        self.rb3 = ResidualBlock(4*self.dim, 2*self.dim, 3, resample='up')
        self.rb4 = ResidualBlock(2*self.dim, 1*self.dim, 3, resample='up')
        self.bn = nn.BatchNorm2d(self.dim)
        #self.conv1 = MyConvo2d(1*self.dim, 3, 3)#for RGB
...
# Via the encoder network, the autoencoder maps information from the input image to the low dimensional space
class Encoder(nn.Module):
    def __init__(self, dim, output_dim, drop_rate=0.0):
        super(Encoder, self).__init__()
        self.dropout = nn.Dropout(drop_rate)
        self.conv_in = nn.Conv2d(1, dim, 3, 1, padding=1)
        # image resizing from 170*170 to 64*64
...
class AE(nn.Module):
    def __init__(self):
        ...
        self.encoder = Encoder(DIM, LATENT_SIZE)
